...
本機開發版已能讓一隻手掌同時覆蓋多個同時出現的 Touch：預設以半徑 0.5 的圓形近似手掌，約占四分之一盤面，另一手可同時按鍵。這不是官方判定或真實手形；Tap、Slide 或不同時間的 Touch 不會自動算入同一掌。畫面上的小型 Touch 多邊形只用來辨識落點，手掌範圍另以大圓虛線表示。公開下載的 v0.1.0 尚未包含本機開發版變更。
